...
class FilterFAB extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    /*
      Your Positioned widget (used in FilterFAB) is being used in a context where its parent does not support StackParentData. Typically, Positioned must be a direct child of a Stack.
      Since the Scaffold already places the floatingActionButton in the correct position, you don't need to wrap your FAB in a Positioned widget inside FilterFAB.
      Solution:
      Remove the Positioned widget from FilterFAB and return the FloatingActionButton directly.
      Explanation:
      The Scaffold's floatingActionButton property automatically positions the FAB.
      Wrapping the FAB with Positioned inside FilterFAB causes the error because its ancestor (the Scaffold) doesn't expect a Positioned widget.
      After this change, your FAB should appear without causing parent data conflicts.
    */
    return
      // Positioned(
      // bottom: 16.0,
...
class _FilterDialogState extends State<FilterDialog> {
  ...
  Widget buildElectionTypeDropdown() {
    ...
    if
    ( widget.role == 'Citizen_specificPreviousElectionViewingResult' || widget.role == "Citizen_electionOverviewReport" || widget.role == "Citizen_electionComparedDetailedReport" ||
      widget.role == "Guest_specificElectionViewingResult" || widget.role == "Guest_electionOverviewReport" || widget.role == "Guest_electionsDetailedReport"
    )
    { validElectionTypes = AppConstants.electionTypes; }
    else if
    (widget.role == 'Party Head'  || widget.role == "Party_Head_View" || widget.role == "PartyHead_specificElectionViewingResult" || widget.role == "PartyHead_specificElectionViewingReport" || widget.role == 'Candidate')
    { validElectionTypes = AppConstants.electionTypesForPartyHead; }
    else if
    (widget.role == 'Candidate_specificElectionViewingResult' || widget.role == "Candidate_specificElectionViewingReport" )
    { validElectionTypes = AppConstants.electionTypesForCandidate; }
    else
    { validElectionTypes = []; }
  ...
  })
  {
    ...
    {
      if
      (widget.role == 'Citizen_specificPreviousElectionViewingResult' || widget.role == "Guest_specificElectionViewingResult" )
      { items = [...AppConstants.statesAndUT_PAN_India, ...AppConstants.statesAndUT];   }
      else if
      (
         widget.role == "Citizen_electionOverviewReport" || widget.role == "Citizen_electionComparedDetailedReport" ||
         widget.role == 'Party Head' || widget.role == "Party_Head_View" ||
         widget.role == "PartyHead_specificElectionViewingResult" || widget.role == "PartyHead_specificElectionViewingReport" ||
         widget.role == 'Candidate'  || widget.role == 'Candidate_specificElectionViewingResult' || widget.role == "Candidate_specificElectionViewingReport" ||
         widget.role == "Guest_electionOverviewReport" || widget.role == "Guest_electionsDetailedReport"
      )
      ...
      content: SingleChildScrollView(
        child: Column(
          ...
          children: [
            ...
            (
              widget.role != "Citizen_specificPreviousElectionViewingResult" && widget.role != "Citizen_electionOverviewReport" && widget.role != "Citizen_electionComparedDetailedReport" &&
              widget.role != 'Party Head' && widget.role != 'Party_Head_View' && widget.role != "PartyHead_specificElectionViewingResult" && widget.role != "PartyHead_specificElectionViewingReport" &&
              widget.role != 'Candidate_specificElectionViewingResult' && widget.role != "Candidate_specificElectionViewingReport" &&
              widget.role != "Guest_specificElectionViewingResult" && widget.role != "Guest_electionOverviewReport" && widget.role != "Guest_electionsDetailedReport"
            )
                DropdownButtonFormField<String>(
            ...
            if
            (
              selectedType != 'Presidential' && selectedType != 'Vice-Presidential' && selectedState != "_PAN India" &&
              widget.role != "Citizen_specificPreviousElectionViewingResult" && widget.role != "Citizen_electionOverviewReport" && widget.role != "Citizen_electionComparedDetailedReport" &&
              widget.role != "PartyHead_specificElectionViewingResult" && widget.role != "PartyHead_specificElectionViewingReport" &&
              widget.role != "Candidate_specificElectionViewingResult" && widget.role != "Candidate_specificElectionViewingReport" &&
              widget.role != "Guest_specificElectionViewingResult" && widget.role != "Guest_electionOverviewReport" && widget.role != "Guest_electionsDetailedReport"
      ...
      actions: [
        ...
        ElevatedButton(
          onPressed: ()
          {
            if
            (
                (selectedYear.isNotEmpty && selectedType.isNotEmpty && selectedState.isNotEmpty  || selectedConstituency.isEmpty)
                &&
                (
                  widget.role == "Citizen_specificPreviousElectionViewingResult"  || widget.role == "Citizen_electionOverviewReport" || widget.role == "Citizen_electionComparedDetailedReport" ||
                  widget.role == "PartyHead_specificElectionViewingResult"|| widget.role == "PartyHead_specificElectionViewingReport" ||
            ...
            (
                ( widget.role == "Party Head" || widget.role == "Party_Head_View" )
                &&
                (selectedYear.isNotEmpty && selectedType.isNotEmpty && selectedState.isNotEmpty && selectedConstituency.isNotEmpty)
            )
            {
              print("1 hiii ................");
              widget.onFilterApplied({
            ...
            else
            {
              ...
              SnackbarUtils.showErrorMessage(context, "Please select all required fields to apply filters.");
            }
