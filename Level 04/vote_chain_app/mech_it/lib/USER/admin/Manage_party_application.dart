...
class _ManagePartyApplicationState extends State<ManagePartyApplication> {
  ...
  Future<void> fetchApplications(String basePath) async
  {
    try
    {
      ...
      (currentStage >= 4 )
      {
        ...
        {
          print("No candidate applications found at: $basePath");
        ...
        // Map each document to a list of applications
        setState(() {
        ...
        print("Fetched applications: $candidateApplications");
      }
    ...
    catch (e)
    {
      ...
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Failed to fetch applications.')), );
      setState(() { isLoading = false; }); // Set loading to false in case of error
    }
  }

  Future<Map<String, dynamic>?> fetchPartyDetails(String state, String partyName) async
  {
    try
    {
      // Check the current election stage
      ...
      {
        setState(() { isLoading = false; });
        SnackbarUtils.showErrorMessage(context, "Party registration phase is on.\nAfter this phase ends you can verify & approve Party-Head applications."); return null;
      }
      else if
      (currentStage >= 4 )
      {
        if (state.isEmpty || partyName.isEmpty)
        {
          print('\n\n\n ****************** \nFFFFetching party details for state: $state, partyName: $partyName');
          return null;
        }
  ...
  Future<void> rejectParty(String applicationId) async
  {
    try
    {
      ...
      else if (currentStage == 4)                                                                                        // Update application status to 'Rejected'
      {
  ...
  Widget buildPartyApplicationCard(Map<String, dynamic> application) {
    return FutureBuilder<Map<String, dynamic>?>(
      future: fetchPartyDetails(ElectionDetails.instance.state ?? '', application['partyName'] ?? ''),
      builder: (context, snapshot) {
        ...
        print("eeeeeeeeeError fetching party details: $partyDetails");
