...
class ElectionResultState extends State<ElectionResult> with TickerProviderStateMixin {
  ...
  Future<void> syncVotes() async {
    ...
    ( currentStage <= 5 )
    {
      SnackbarUtils.showErrorMessage(context, "Voting hasn't started yet.\nSo no data is available to sync.");
      return;
    }
    else if
    ( currentStage == 6 )
    {
      SnackbarUtils.showErrorMessage(context, "Voting hasn't started yet.\nSo, only constituency, party & candidate info is available as of now if it is.");
      _tabController.animateTo(1);
      return;
    }
    else if
...
class _ElectionResultSecondTabState extends State<ElectionResultSecondTab> {
  ...
  Widget build(BuildContext context) {
    ...
    print("\n*** 00 ${electionDetails.electionType}");

    return ElectionResultScreen(
      electionType: "${electionDetails.electionType}", // or any dynamic value
      state: "${electionDetails.state}",                   // example state
      year: "${electionDetails.year}",                     // example year
      role: "Admin",
    );
  }
}
